...
export const AllItems: FunctionComponent<{
	items: ReadonlyArray<SearchIssuesAndPullRequestsResponseItemsItem>;
}> = ({ items: rawItems }) => {
	...
	const hasManyRepos = items.some(
		(item) => item.repository_url !== items[0].repository_url,
	);
	const renderItem = (item: SearchIssuesAndPullRequestsResponseItemsItem) => (
		<Item
			key={item.id}
			item={item}
			hide={() => setHidden([...hidden, item.id])}
			showRepo={hasManyRepos}
		/>
	);
...
const Label: FunctionComponent<{
	label: SearchIssuesAndPullRequestsResponseItemsItemLabelsItem;
}> = ({ label }) => (
	<span
		className="label"
		key={label.id}
		style={{ backgroundColor: `#${label.color}` }}>
		<a style={{ color: getContrastColor(label.color) }}>
			{withEmoji(label.name)}
		</a>
	</span>
);
